...
const Workspace = {
 ...
 db: async function (tracing = true) {
  ...
  console.log({
   filename: `${!!process.env.STORAGE_DIR ? `${process.env.STORAGE_DIR}/` : "storage/"}anythingllm.db`,
   driver: sqlite3.Database,
  });
  const db = await open({
   filename: `${!!process.env.STORAGE_DIR ? `${process.env.STORAGE_DIR}/` : ""}anythingllm.db`,
   driver: sqlite3.Database,
  });
  console.log
 ...
 new: async function (name = null) {
  ...
  const { id, success, message } = await db
   .run(`INSERT INTO ${this.tablename} (name, slug) VALUES (?, ?)`, [name, slug])
   .then((res) => {
 ...
 update: async function (id = null, data = {}) {
  ...
  if (validKeys.length === 0 || validKeys.length !== values.length) return { workspace: { id }, message: "No valid fields to update!" };
