...
export class IUser {
    static async signUp(data: any) {
        try {
            let admin = await Users.findOne({ where: { email: data['email'].toLowerCase() }, raw: true });
            if (admin) return { status: statusCode.ALREADY_EXIST, message: `${MODULE_NAME.EMAIL} already exists` };
            ...
            return { status: statusCode.OK, message: l10n.t('COMMON_SUCCESS', { key: MODULE_NAME.USER, method: REQUEST_METHOD.REGISTERED }) };
    ...
    static async signIn(data: any) {
        try {
            const attributes = ['id', 'email', 'userId', 'firstName', 'gender', 'isActive', 'createdAt'];
            let user = await Users.findOne({ where: { email: data.email, password: data.password }, attributes });
    ...
    static async verifyPassword(data: any) {
        try {
            return { status: statusCode.OK, message: `${MODULE_NAME.USER} ${MODULE_NAME.EMAIL} verified successfully` };
        } catch (error) {
            console.error(error);
            return { status: statusCode.INTERNAL_SERVER_ERROR, message: 'SOMETHING_WENT_WRONG' };
        }
    ...
    static async getProfile(data: any) {
        try {
